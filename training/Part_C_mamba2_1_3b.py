...
class QuestMamba(nn.Module):
    def __init__(self, model_name, num_cats, cat_emb_dim=16, tokenizer_len=None):
        ...
        # --- Config Overrides based on Model Size ---
        if '780m' in model_name:
            ...
            self.config.vocab_size = 50277
            self.config.n_groups = 1
        elif '1.3b' in model_name:
            # Standard Mamba2-1.3B Config
            self.config.hidden_size = 2048
            self.config.d_model = 2048
            self.config.num_heads = 64
            self.config.head_dim = 64
            self.config.n_layer = 48
            self.config.vocab_size = 50277 
            # 1.3B usually uses n_groups=1 by default, but safe to enforce
            self.config.n_groups = 1 
    ...
    def forward(self, ids, mask, cats):
        ...
        # --- [OPTIMIZATION] Last Token Pooling ---
        # Instead of Mean Pooling, we take the embedding of the last non-padding token.
        # This token has "read" the entire sequence (Causal property).
        
        # mask is [Batch, Len], we want the index of the last '1'
        last_token_indices = mask.sum(1) - 1
        # Gather the hidden state at these indices
        batch_size = hidden_states.shape[0]
        text_feature = hidden_states[torch.arange(batch_size, device=hidden_states.device), last_token_indices]
